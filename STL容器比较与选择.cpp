...
3. deque 双端队列，支持快速随机存取(时间复杂度为O(1))，查询时间复杂度是O(n)，头部尾部插入\删除速度快(时间复杂度O(1))；
4. array 固定大小数组，支持快速随机存取(时间复杂度O(1)),不支持插入\删除操作；
5. list 双向链表，支持双向顺序访问，任意位置插入\删除(知道目标节点指针时)时间复杂度是o(1)，但是查询和随机访问(知道索引)的时间复杂度是O(1);
6. forward_list 单向链表，支持单向顺序访问，任意位置插入\删除(知道目标节点指针时)时间复杂度是o(1)，但是查询和随机访问(知道索引)的时间复杂度是O(1);

二、顺序容器适配器
1. stack 栈，不支持快速随机存取(不支持下标访问)，无迭代器，不支持查询，头部插入\删除速度快(时间复杂度O(1))，具有先进后出的特点；
2. queue 队列，不支持快速随机存取(不支持下标访问)，无迭代器，不支持查询，尾部插入\头部删除速度快(时间复杂度O(1))，具有先进先出的特点；
3. priority_queue 优先队列(头文件concurrent_priority_queue.h)，不支持快速随机存取(不支持下标访问)，无迭代器，不支持查询；
...
3. unoreder_set 用哈希函数组织的set，unorder版本的map和set只提供前向迭代器（非unorder版本提供双向迭代器）
   对于unordered_set，insert/find/erase的平均复杂度是O(1)，但是最坏复杂度是O(N)的，这里N是指容器中元素数
   有两种情况会出现O(N)复杂度。
...
4. 对于unordered_map，insert/find/erase的平均复杂度是O(1)